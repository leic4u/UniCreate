  ...
  return (
    <div className="space-y-7">
      <div>
        ...
        <h2 className="text-xl font-semibold tracking-tight">{isUpdate ? "Update Installer" : "Add Installers"}</h2>
      ...
      {!isUpdate && (userRepos.loading || userRepos.repos.length > 0) && (
        <div className="space-y-2">
          <span className="flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground">
            <Github className="h-3 w-3" />Your repositories
          </span>
          {userRepos.loading ? (
                    ...
                    {userRepos.expandedRepo === key && userRepos.releases[key] && (
                      <div className="border-t border-border/30 px-3 py-2 space-y-2 animate-fade-in">
                        {userRepos.releases[key].length === 0 ? (
                          <p className="text-[11px] text-muted-foreground">No releases with installers found.</p>
                        ) : (
        ...
        {isDragging ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="flex items-center gap-1.5 text-[12px] font-medium text-foreground"><Cpu className="h-3 w-3" />Architecture</label>
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-1.5 text-[12px] font-medium text-foreground"><Box className="h-3 w-3" />Installer Type</label>
            ...
            {localHash && (
              <div className="rounded-lg border border-emerald-500/15 bg-emerald-500/5 p-3 space-y-2 animate-fade-in">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-medium text-emerald-400">Local file: {localHash.fileName}</span>
                </div>
                ...
                <button onClick={handleAddFromLocal} disabled={!url.trim()}
                  ...
                  <Plus className="h-3 w-3" />Add with local hash
                </button>
              </div>
            )}
            ...
            {!localHash && (
              <button onClick={handleAnalyze} disabled={!url.trim() || isAnalyzing} data-action="primary"
                ...
                {isAnalyzing
                  ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Downloading & computing hash...</>
                  : <><Plus className="h-3.5 w-3.5" />Analyze & Add</>}
              </button>
            )}

            <p className="text-center text-[11px] text-muted-foreground">or drag & drop a local file to compute hash</p>
      ...
      {manifest.installers.length > 0 && (
        <div className="space-y-2.5 animate-fade-in">
          ...
          {manifest.installers.map((installer, index) => (
            <div key={index} className="group flex items-start gap-3 rounded-lg border border-border bg-card/30 p-3.5 transition-colors hover:bg-card/60">
      ...
      {autoFilled && (
        <div className="flex items-center gap-2 rounded-lg border border-primary/15 bg-primary/5 px-4 py-2.5 animate-fade-in">
          ...
          <span className="text-[12px] font-medium text-primary/80">Metadata auto-filled from GitHub repository. You can review and edit in the next step.</span>
        </div>
      )}
